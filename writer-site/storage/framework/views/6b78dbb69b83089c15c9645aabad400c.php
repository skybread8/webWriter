...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.layout','data' => ['title' => 'Pedidos']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Pedidos']); ?>
    <div class="space-y-8">
        <div>
        ...
        <div class="grid gap-4 md:grid-cols-4">
            <div class="border border-zinc-800 rounded-2xl p-4 bg-zinc-900/40">
                ...
                <div class="text-2xl font-semibold"><?php echo e(number_format($totalOrders)); ?></div>
            </div>
            <div class="border border-zinc-800 rounded-2xl p-4 bg-zinc-900/40">
                <div class="text-xs text-zinc-500 mb-1">Pendientes de envío</div>
                <div class="text-2xl font-semibold text-amber-400"><?php echo e(number_format($pendingShipments)); ?></div>
            </div>
            <div class="border border-zinc-800 rounded-2xl p-4 bg-zinc-900/40">
                <div class="text-xs text-zinc-500 mb-1">Enviados</div>
                <div class="text-2xl font-semibold text-green-400"><?php echo e(number_format($shippedOrders)); ?></div>
            </div>
            <div class="border border-zinc-800 rounded-2xl p-4 bg-zinc-900/40">
                <div class="text-xs text-zinc-500 mb-1">Devoluciones</div>
                <div class="text-2xl font-semibold text-red-400"><?php echo e(number_format($refundedOrders)); ?></div>
            </div>
        </div>
        ...
        <div class="border border-zinc-800 rounded-2xl p-6 bg-zinc-900/40">
            <form method="GET" action="<?php echo e(route('admin.orders.index')); ?>" class="space-y-4">
                <div class="grid gap-4 md:grid-cols-4">
                    ...
                    <div>
                        ...
                        <select name="shipped" class="w-full px-3 py-2 bg-zinc-950 border border-zinc-800 rounded-lg text-sm text-zinc-100 focus:ring-1 focus:ring-zinc-500 focus:border-zinc-500">
                            <option value="">Todos</option>
                            <option value="0" <?php echo e(request('shipped') === '0' ? 'selected' : ''); ?>>No enviado</option>
                            <option value="1" <?php echo e(request('shipped') === '1' ? 'selected' : ''); ?>>Enviado</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="refunded" class="w-full px-3 py-2 bg-zinc-950 border border-zinc-800 rounded-lg text-sm text-zinc-100 focus:ring-1 focus:ring-zinc-500 focus:border-zinc-500">
                            <option value="">Todos</option>
                            <option value="0" <?php echo e(request('refunded') === '0' ? 'selected' : ''); ?>>Sin devolución</option>
                            <option value="1" <?php echo e(request('refunded') === '1' ? 'selected' : ''); ?>>Con devolución</option>
                        </select>
                    </div>
                </div>
                <div class="flex gap-2">
                    ...
                    <a href="<?php echo e(route('admin.orders.index')); ?>" class="px-4 py-2 bg-zinc-800 text-zinc-200 text-xs font-semibold rounded-lg hover:bg-zinc-700 transition-colors">
                        Limpiar
                    </a>
                </div>
            </form>
                        ...
                        <tbody>
                            <?php $__currentLoopData = $orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="border-b border-zinc-800/50 hover:bg-zinc-900/60">
                                    <td class="py-3 px-4">
                                        <div class="font-semibold text-zinc-200"><?php echo e($order->order_number); ?></div>
                                    </td>
                                    <td class="py-3 px-4">
                                        <div class="text-zinc-300"><?php echo e($order->customer_name ?? $order->user?->name ?? 'N/A'); ?></div>
                                        <div class="text-xs text-zinc-500"><?php echo e($order->customer_email ?? $order->user?->email ?? 'N/A'); ?></div>
                                    </td>
                                    <td class="py-3 px-4">
                                        <div class="text-zinc-400 text-xs"><?php echo e($order->created_at->format('d/m/Y H:i')); ?></div>
                                    </td>
                                    <td class="py-3 px-4">
                                        <div class="font-semibold text-amber-400"><?php echo e(number_format($order->total, 2)); ?> €</div>
                                    </td>
                                    <td class="py-3 px-4">
                                        <?php
                                            $statusColors = [
                                        ...
                                        ?>
                                        <span class="px-2 py-1 text-xs font-semibold rounded-full <?php echo e($statusColors[$order->status] ?? 'bg-zinc-500/20 text-zinc-400'); ?>">
                                    ...
                                    <td class="py-3 px-4">
                                        <?php if($order->shipped): ?>
                                            ...
                                            <?php if($order->shipped_at): ?>
                                                <div class="text-xs text-zinc-500 mt-1"><?php echo e($order->shipped_at->format('d/m/Y')); ?></div>
                                            <?php endif; ?>
                                        <?php else: ?>
                                    ...
                                    <td class="py-3 px-4">
                                        <a href="<?php echo e(route('admin.orders.show', $order)); ?>" class="px-3 py-1.5 bg-zinc-100 text-zinc-950 text-xs font-semibold rounded-lg hover:bg-white transition-colors">
                                            Ver detalles
                                        </a>
                                    </td>
                                </tr>
...
<?php endif; ?>
<?php /**PATH /Users/gerardrevo/Documents/GitHub/WebKevin/writer-site/resources/views/admin/orders/index.blade.php ENDPATH**/ ?>