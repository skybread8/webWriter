                            ...
                            <?php if($allImages->isNotEmpty()): ?>
                                ...
                                >
                                    ...
                                    <?php if($allImages->count() > 1): ?>
                                        <div class="absolute inset-0 pointer-events-none" style="z-index: 999;">
                                            <button 
                                                onclick="const carousel = this.closest('[x-data]'); if(carousel && carousel.__x) { carousel.__x.$data.prev(); } event.stopPropagation(); event.preventDefault(); return false;"
                                                class="absolute left-2 top-1/2 -translate-y-1/2 z-50 w-10 h-10 rounded-full bg-zinc-900/98 hover:bg-zinc-800 border-2 border-zinc-600 flex items-center justify-center text-zinc-100 transition-all shadow-xl hover:scale-110 pointer-events-auto"
                                            ...
                                            <button 
                                                onclick="const carousel = this.closest('[x-data]'); if(carousel && carousel.__x) { carousel.__x.$data.next(); } event.stopPropagation(); event.preventDefault(); return false;"
                                                class="absolute right-2 top-1/2 -translate-y-1/2 z-50 w-10 h-10 rounded-full bg-zinc-900/98 hover:bg-zinc-800 border-2 border-zinc-600 flex items-center justify-center text-zinc-100 transition-all shadow-xl hover:scale-110 pointer-events-auto"
                                            ...
                                            <div class="absolute bottom-2 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1.5 pointer-events-auto">
                                                <template x-for="(image, index) in images" :key="index">
                                                    <button 
                                                        :onclick="'const carousel = this.closest(\'[x-data]\'); if(carousel && carousel.__x) { carousel.__x.$data.currentIndex = ' + index + '; } event.stopPropagation(); event.preventDefault(); return false;'"
                                                        class="w-1.5 h-1.5 rounded-full transition-all"
                                                        :class="currentIndex === index ? 'bg-amber-400 w-4' : 'bg-zinc-600 hover:bg-zinc-500'"
