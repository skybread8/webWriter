...
@if($settings?->cookies_enabled ?? true)
    <div 
        x-data="{ 
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                <div class="flex-1">
                    <h3 id="cookie-banner-title" class="text-sm font-semibold text-zinc-100 mb-2">
                        {{ __('common.cookies.title') }}
                    </h3>
                    <p id="cookie-banner-description" class="text-xs text-zinc-400 leading-relaxed">
                        {{ __('common.cookies.description') }}
                        <a href="{{ localized_route('legal.cookies') }}" class="underline hover:text-zinc-300 transition-colors">
                            {{ __('common.cookies.learn_more') }}
                        </a>
                    </p>
                </div>
                <div class="flex items-center gap-3 shrink-0">
                    <button
                        ...
                        aria-label="{{ __('common.cookies.reject') }}"
                    >
                        {{ __('common.cookies.reject') }}
                    </button>
                    <button
                        @click="accept()"
                        class="px-6 py-2 text-xs font-semibold bg-zinc-100 text-zinc-950 rounded-lg hover:bg-white transition-colors"
                        aria-label="{{ __('common.cookies.accept') }}"
                    >
                        {{ __('common.cookies.accept') }}
                    </button>
                </div>
            </div>
        </div>
