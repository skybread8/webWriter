...
@section('content')
    ...
    >
        <div class="relative min-h-[85vh] sm:min-h-[90vh] flex items-center">
            ...
            <div class="relative z-10 px-5 sm:px-8 py-20 sm:py-32 max-w-6xl mx-auto w-full">
                <div class="space-y-8 max-w-3xl" 
                     x-show="show" 
                     x-transition:enter="transition ease-out duration-1000"
                     x-transition:enter-start="opacity-0 translate-y-8"
                     x-transition:enter-end="opacity-100 translate-y-0">
                    <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-900/60 border border-zinc-800/50 backdrop-blur-sm">
                        <x-icons.book class="w-4 h-4 text-amber-400" />
                        <p class="text-[11px] tracking-[0.35em] uppercase text-zinc-300">
                            Escritor independiente
                        </p>
                    </div>
                    <h1 class="font-['DM_Serif_Display'] text-5xl sm:text-6xl md:text-7xl lg:text-8xl tracking-tight leading-[0.95]">
                    ...
                    <div class="pt-4 flex flex-wrap items-center gap-4">
                        <a href="{{ route('books.index.public') }}" class="group inline-flex items-center gap-2 px-6 py-3 rounded-full bg-zinc-100 text-zinc-950 text-sm font-semibold tracking-wide uppercase hover:bg-white transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-zinc-100/20">
                            <span>Explorar libros</span>
                            <x-icons.arrow-right class="w-4 h-4 transition-transform group-hover:translate-x-1" />
                        </a>
                        <a href="{{ route('about') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-zinc-700 text-zinc-300 text-sm font-medium tracking-wide uppercase hover:border-zinc-600 hover:text-zinc-100 hover:bg-zinc-900/40 transition-all duration-300">
                            <x-icons.user class="w-4 h-4" />
                            <span>Sobre Kevin</span>
                        </a>
                    </div>
                </div>
            </div>
    ...
    @if($books->isNotEmpty())
        <section 
            ...
            class="px-5 sm:px-8 py-20 sm:py-28 border-t border-zinc-800/50 bg-gradient-to-b from-zinc-950 to-zinc-900/50"
        >
            <div class="max-w-7xl mx-auto">
                <div 
                    class="flex items-center justify-between gap-4 mb-12"
                ...
                >
                    <div>
                        ...
                        <p class="mt-3 text-sm text-zinc-400 max-w-md">
                            Selección de obras disponibles para lectura inmediata
                        </p>
                    </div>
                    <a href="{{ route('books.index.public') }}" class="hidden sm:flex items-center gap-2 text-[11px] tracking-[0.25em] uppercase text-zinc-400 hover:text-zinc-100 transition-colors group">
                        <span>Ver todos</span>
                        <x-icons.arrow-right class="w-4 h-4 transition-transform group-hover:translate-x-1" />
                    </a>
                </div>
                ...
                <div class="mt-10 text-center sm:hidden">
                    <a href="{{ route('books.index.public') }}" class="inline-flex items-center gap-2 text-[11px] tracking-[0.25em] uppercase text-zinc-400 hover:text-zinc-100 transition-colors">
    ...
    @php
        $testimonials = \App\Models\Testimonial::where('active', true)->orderBy('created_at', 'desc')->take(3)->get();
    @endphp
            ...
            <div class="max-w-7xl mx-auto">
                ...
                >
                    ...
                    <p class="mt-3 text-sm text-zinc-400 max-w-md mx-auto">
                        Reseñas de quienes han leído las obras de Kevin Perez
                    </p>
                </div>
        ...
        <div class="max-w-4xl mx-auto text-center">
            ...
            >
                ...
                <p class="text-base text-zinc-400 max-w-xl mx-auto">
                    Descubre historias que te acompañarán mucho después de cerrar el libro
                </p>
                <div class="pt-4 flex flex-wrap items-center justify-center gap-4">
                    <a href="{{ route('books.index.public') }}" class="group inline-flex items-center gap-2 px-8 py-4 rounded-full bg-zinc-100 text-zinc-950 text-sm font-semibold tracking-wide uppercase hover:bg-white transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-zinc-100/20">
                        <x-icons.book class="w-5 h-5" />
                        <span>Ver catálogo completo</span>
                        <x-icons.arrow-right class="w-4 h-4 transition-transform group-hover:translate-x-1" />
                    </a>
                    <a href="{{ route('contact') }}" class="inline-flex items-center gap-2 px-8 py-4 rounded-full border border-zinc-700 text-zinc-300 text-sm font-medium tracking-wide uppercase hover:border-zinc-600 hover:text-zinc-100 hover:bg-zinc-900/40 transition-all duration-300">
